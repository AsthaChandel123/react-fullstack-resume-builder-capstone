...
<div class="report-page">
  ...
  <p>ResumeAI&rsquo;s Saathi engine implements a pure-LLM slot-filling architecture. Every user turn is sent to Gemma 3 27b as the primary extractor and conversational responder, with Gemini 2.5 Flash configured as an automatic backup invoked only when Gemma fails or times out. Both calls receive Saathi&rsquo;s most recent question and the list of currently-missing slots so that bare one-word replies (for example, &ldquo;Shimla&rdquo; in answer to &ldquo;Where are you based?&rdquo;) are correctly mapped to the intended field. A single configuration file, <code data-type="file">src/saathi/engine/modelConfig.ts</code>, is the only place the model identifiers appear, ensuring upgrades require a single edit.</p>
  ...
  <p>The Web Crypto API, available in all modern browsers, provides hardware-accelerated implementations of SHA-256 and HMAC operations. ResumeAI uses <code data-type="fn">crypto.subtle.digest('SHA-256', ...)</code> for resume content hashing and delegates HMAC-SHA256 signing to Firebase Cloud Functions, ensuring that per-criteria secrets remain server-side.</p>
...
<div class="report-page">
  ...
  <table>
    ...
    <tbody>
      ...
      <tr><td>FR-3</td><td>The system shall extract structured entities (name, email, phone, location, degree, institution, year, field, GPA, company, role, dates, bullets, skills, LinkedIn, GitHub) from natural multilingual text using a Gemma 3 27b primary model with Gemini 2.5 Flash automatic backup, with no regex pattern matching in the user-facing path.</td><td>Saathi</td></tr>
  ...
  <p>The frontend is organized into five primary modules, each encapsulated in its own directory with dedicated state management, business logic, and UI components: (1) <code data-type="file">src/saathi/</code> for the conversational engine, (2) <code data-type="file">src/ai/</code> for the employer screening pipeline, (3) <code data-type="file">src/bridge/</code> for the trust layer, (4) <code data-type="file">src/wellbeing/</code> for the wellbeing engine, and (5) <code data-type="file">src/builder/</code> for the form-based resume editor. Shared state is managed through Zustand stores (<code data-type="file">src/store/</code>) with IndexedDB persistence via a custom adapter (<code data-type="file">src/store/persist.ts</code>).</p>
  ...
  <p>The Saathi engine implements a slot-filling dialogue architecture defined in <code data-type="file">src/saathi/engine/slots.ts</code>. The system defines 23 data slots organized into two priority tiers: 8 required slots (name, email, phone, location, degree, institution, graduation year, field of study) and 15 preferred slots (GPA, company, role, dates, bullets, project name, project tech, project outcome, skills, certifications, summary, LinkedIn, GitHub, relocation preference, target role).</p>

  <p>Conversation flow proceeds through 7 phases defined by the <code data-type="type">ConversationPhase</code> type: warmup, education, experience, projects, skills, wrapup, and review. Phase transitions are governed by the <code data-type="fn">updatePhase()</code> function in <code data-type="file">src/saathi/engine/slotMachine.ts</code>, which examines slot completion state to determine the appropriate next phase. Users may skip non-required phases through negation, which is detected by the language model during extraction (<code>isNegation</code> field of <code data-type="type">AIExtractedData</code>) rather than through static keyword matching, allowing the system to recognise multilingual refusals such as &ldquo;nahi hai bhai kuch nahi&rdquo; or &ldquo;skip kar dijiye&rdquo;.</p>

  <p>The engine runs a single asynchronous pipeline, <code data-type="fn">processUserInputAsync()</code>, which invokes <code data-type="fn">extractWithAI()</code> in <code data-type="file">src/saathi/engine/aiExtractor.ts</code>. The extractor calls Gemma 3 27b first; if Gemma errors or exceeds its 15-second deadline, the same prompt is retried against Gemini 2.5 Flash. Gemini calls use the structured JSON response schema and disable the model&rsquo;s thinking budget for predictable latency, while Gemma calls use prompt-based JSON instructions because the Generative Language API does not expose <code>responseMimeType</code> for Gemma. The extracted <code data-type="type">AIExtractedData</code> object is then merged into the slot state by <code data-type="fn">fillSlotsFromAI()</code>.</p>

  <p>Response generation lives in <code data-type="file">src/saathi/engine/aiResponseGenerator.ts</code>. Each turn the full conversation history together with the lists of filled and still-missing slots are passed to the model with an explicit instruction never to repeat a question already asked, eliminating the failure mode in which a missed slot extraction caused the assistant to ask the user for the same field on every subsequent turn.</p>
  ...
  <p>The employer screening pipeline is implemented as a directed acyclic graph of 9 specialized agents, defined in <code data-type="file">src/ai/orchestrator/agenticPipeline.ts</code>. The DAG executor in <code data-type="file">src/ai/orchestrator/dagExecutor.ts</code> implements Kahn&rsquo;s topological sort algorithm for cycle detection and parallel execution scheduling.</p>
  ...
  <p>The scoring formula, implemented in <code data-type="file">src/ai/agents/ScoreAgent.ts</code>, computes a 9-parameter weighted composite: skills (30%, NACE 2024), experience (20%, NACE Internship Survey), education (15%, NACE 2024), projects (10%, AAC&amp;U/Hart 2018), certifications (5%, SHRM 2021), distance (5%, Marinescu &amp; Rathelot 2018), extracurricular (5%, Roulin &amp; Bangerter 2013), GPA (3%, NACE 2024), and completeness (2%, Ladders 2018). When distance data is unavailable, the 5% weight is redistributed proportionally across remaining parameters.</p>
  ...
  <p>Employers publish criteria through the <code data-type="type">BridgeCriteria</code> interface (defined in <code data-type="file">src/bridge/types.ts</code>), specifying required and preferred skills, custom scoring signals, weight overrides, threshold scores, and test configuration including difficulty floor and question count. Each criteria set receives a unique short code for candidate reference.</p>

  <p>Before testing, the candidate&rsquo;s resume is pinned: <code data-type="fn">createResumePin()</code> in <code data-type="file">src/bridge/resumePin.ts</code> extracts resume text, normalizes it (lowercase, strip non-alphanumeric), computes a SHA-256 hash, and records the scoring state at test time. Post-test modification detection uses n-gram Jaccard similarity (<code data-type="fn">computeChangePct()</code>) to classify changes as same (&lt;10%), moderate (10&ndash;30%), or substantial (&gt;30%).</p>

  <p>The adaptive testing engine generates questions at 5 difficulty levels (Fundamentals, Applied, Architecture &amp; Edge Cases, Expert Tradeoffs, Novel Problem) with level multipliers (1.0, 1.5, 2.5, 4.0, 6.0) and score ceilings (L2: 45, L3: 75, L4: 90, L5: 100). Questions are validated through anti-LLM-tell constraints enforced by <code data-type="fn">validateQuestion()</code> in <code data-type="file">src/bridge/test/questionGenerator.ts</code>.</p>
  ...
  <table>
    ...
    <tbody>
      <tr><td><code data-type="str">criteria</code></td><td>BridgeCriteria (shortCode, jobTitle, requiredSkills, weights, testConfig, etc.)</td><td>Employer-published assessment criteria</td></tr>
      <tr><td><code data-type="str">scorecards</code></td><td>SignedScorecard (version, candidateId, sessionId, resumePin, verification, integrity, signature)</td><td>Cryptographically signed test results</td></tr>
      <tr><td><code data-type="str">matches</code></td><td>MatchSignal (criteriaCode, scorecardId, candidateId, employerId, scores, gap, status)</td><td>Candidate-employer match signals</td></tr>
...
<div class="report-page">
  ...
  <p>The slot-filling engine is implemented in <code data-type="file">src/saathi/engine/slotMachine.ts</code> (818 lines). The core data structure is <code data-type="type">SlotState</code>, a TypeScript interface containing: a <code data-type="type">Map&lt;SlotId, string | string[]&gt;</code> for slot values, a <code data-type="type">ConversationPhase</code> enum for current phase, a <code data-type="type">Map&lt;string, number&gt;</code> for array indices (supporting multiple entries per section), and a <code data-type="type">Set&lt;SlotId&gt;</code> for explicitly skipped slots.</p>

  <p>Each user message is processed through the following pipeline: (1) input sanitization (2000-character limit, control character removal), (2) LLM extraction via <code data-type="fn">extractWithAI()</code>, which is given the user&rsquo;s reply, the full conversation context, the current phase, Saathi&rsquo;s most recent question, and the current list of missing slots, (3) slot filling through <code data-type="fn">fillSlotsFromAI()</code> which maps the structured response onto slot IDs, (4) phase update via <code data-type="fn">updatePhase()</code>, and (5) response generation via <code data-type="fn">generateSaathiResponse()</code> which receives the entire turn-by-turn history and produces a one-to-two-sentence reply that advances the conversation without repeating itself.</p>

  <p>The engine supports multiple entries for experience, projects, and education through an array entry system. The <code data-type="fn">addArrayEntry()</code> function in <code data-type="file">src/saathi/engine/slots.ts</code> stores each entry as a JSON-serialized object in a string array under a synthetic slot key (e.g., <code data-type="str">experience[]._entries</code>). The <code data-type="fn">getArrayEntries()</code> function retrieves and parses these entries for resume generation.</p>

  <h4 class="subsection-title">5.1.2 LLM Entity Extraction</h4>

  <p>Entity extraction in <code data-type="file">src/saathi/engine/aiExtractor.ts</code> sends the user message, conversation context, current phase, the most recent Saathi question, and the list of currently-missing slots to the configured language model. The model returns a single <code data-type="type">AIExtractedData</code> JSON object whose fields cover identity (name, email, phone, location, LinkedIn, GitHub), education (degree, institution, year, field, GPA), work history (company, role, dates, bullets), skills, target role, and three intent flags (<code>isNegation</code>, <code>isConfusion</code>, <code>isOffTopic</code>) that the slot machine uses to drive phase transitions and clarification responses.</p>

  <p>Model selection is centralised in <code data-type="file">src/saathi/engine/modelConfig.ts</code>, which exports a single <code>SAATHI_MODELS</code> constant declaring the primary and backup model identifiers along with helper functions that determine whether a given model accepts the Generative Language API&rsquo;s structured JSON parameters. Allowed values are restricted to the Gemma 3/4 family and Gemini 2.5/3, providing a single, auditable point of control for upgrades.</p>

  <h4 class="subsection-title">5.1.3 Multilingual Voice Input</h4>

  <p>Voice input is handled by <code data-type="file">src/saathi/voice/speechInput.ts</code> using the Web Speech API. Language detection is performed by <code data-type="file">src/saathi/voice/languageDetect.ts</code>, which examines Unicode block ranges to identify 10 scripts: Latin (en-IN), Devanagari (hi-IN), Tamil (ta-IN), Bengali (bn-IN), Telugu (te-IN), Malayalam (ml-IN), Gujarati (gu-IN), Kannada (kn-IN), Punjabi (pa-IN), and Odia (or-IN). The <code data-type="fn">detectScript()</code> function counts character occurrences in each Unicode range and returns the dominant script, which is mapped to a BCP-47 language tag via <code data-type="fn">getSpeechLang()</code> for the speech recognizer.</p>

  <h4 class="subsection-title">5.1.4 LLM Response Generation</h4>

  <p>Each user turn invokes two model calls in sequence inside <code data-type="fn">processUserInputAsync()</code>. <code data-type="fn">extractWithAI()</code> sends the message together with the conversation context, current phase, last Saathi question, and missing-slot list, and receives a structured <code data-type="type">AIExtractedData</code> object. <code data-type="fn">generateSaathiResponse()</code> then receives the entire turn history together with the filled and missing slot lists and produces the next reply, which is constrained never to repeat a question that already appears in the history. Both functions try Gemma 3 27b first and transparently fall back to Gemini 2.5 Flash on any error, after which the user-visible chat surfaces a friendly retry message rather than a stack trace.</p>

  <h4 class="subsection-title">5.1.5 Resume Generation</h4>

  <p>The <code data-type="fn">slotsToResume()</code> function in <code data-type="file">src/saathi/engine/resumeGenerator.ts</code> converts the filled <code data-type="type">SlotState</code> into a <code data-type="type">Resume</code> object compatible with the resume store. It processes array entries for education, experience, and projects through <code data-type="fn">getArrayEntries()</code>, with fallback to flat slot values for single-entry cases. The resulting Resume object is structured with typed sections (education, experience, projects, skills, certifications, extracurricular), each containing entries with fields and bullets.</p>

  <h3 class="section-title">5.2 Resume Builder (Form Mode)</h3>

  <h4 class="subsection-title">5.2.1 Template System</h4>

  <p>Four resume templates are implemented as React components in <code data-type="file">src/builder/templates/</code>: ATS Classic (<code data-type="file">ATSClassic.tsx</code>) optimized for ATS parsing with standard section headings and clean formatting; Modern Blue (<code data-type="file">ModernBlue.tsx</code>) with a two-column layout and accent colors; Creative (<code data-type="file">Creative.tsx</code>) with a sidebar design for visual impact; and Minimal (<code data-type="file">Minimal.tsx</code>) with maximum content density. All templates receive the same <code data-type="type">Resume</code> prop and render identically structured content with different visual treatments.</p>

  <h4 class="subsection-title">5.2.2 Drag-and-Drop Section Reordering</h4>

  <p>Section reordering is implemented using <code>@dnd-kit/core</code> and <code>@dnd-kit/sortable</code> in <code data-type="file">src/builder/components/DraggableSections.tsx</code>. Users can reorder resume sections (education, experience, projects, skills, certifications, extracurricular) through drag-and-drop interaction. The sort order persists in the resume store.</p>

  <h4 class="subsection-title">5.2.3 AI Coach Panel</h4>

  <p>The AI Coach Panel (<code data-type="file">src/builder/components/AICoachPanel.tsx</code>) provides real-time feedback on resume quality. It integrates with the employer scoring pipeline to show how the current resume would score against a sample job description. Suggestions cover missing sections, weak phrasing, lack of quantified achievements, and skills gaps, with each suggestion citing its research source.</p>
...
<div class="report-page">
  ...
  <p>The JD Processor agent (<code data-type="file">src/ai/agents/JDAgent.ts</code>) extracts structured requirements from raw job description text. It identifies the job title, seniority level, required skills, preferred skills, and minimum qualifications using regex pattern matching and keyword extraction. Extracted skills are normalized through the skills taxonomy (<code data-type="file">src/ai/taxonomy/skillsGraph.ts</code>) to canonical forms (e.g., &ldquo;JS&rdquo; normalizes to &ldquo;JavaScript&rdquo;, &ldquo;k8s&rdquo; to &ldquo;Kubernetes&rdquo;). The structured output feeds both the Skills Matcher and the scoring formula.</p>

  <h4 class="subsection-title">5.3.2 DAG Orchestrator</h4>

  <p>The DAG executor (<code data-type="file">src/ai/orchestrator/dagExecutor.ts</code>) implements a parallel execution engine for the agent graph. The <code data-type="fn">validateDAG()</code> function uses Kahn&rsquo;s algorithm: it computes in-degree for each node, seeds a queue with zero-in-degree nodes, and iteratively processes the queue while decrementing neighbor in-degrees. If the resulting topological order contains fewer nodes than the input, a cycle is detected and the pipeline aborts.</p>

  <p>Execution proceeds through the <code data-type="fn">executeDAG()</code> function: (1) find all nodes whose dependencies are satisfied, (2) execute them in parallel via <code data-type="type">Promise.allSettled</code>, (3) on completion, mark the node as completed and re-check for newly runnable nodes, (4) repeat until no nodes are running or pending. Failed optional nodes are logged but do not block dependents. Failed required nodes cause their dependents to be skipped with a &ldquo;Required dependency failed&rdquo; trace entry.</p>

  <h4 class="subsection-title">5.3.3 Scoring Formula</h4>

  <p>The <code data-type="fn">computeScore()</code> function in <code data-type="file">src/ai/agents/ScoreAgent.ts</code> implements the composite scoring formula:</p>
  ...
  <p>The L3 Reasoning Agent (<code data-type="file">src/ai/agents/L3_ReasonAgent.ts</code>) uses Henle et al.&rsquo;s (2019) three-dimensional taxonomy to classify detected inconsistencies. The contradiction detection prompt instructs the model to analyze for fabrication (knowingly false information), embellishment (exaggerated experience), and omission (strategically hidden information). Each red flag carries a penalty value and an evidence string referencing the specific resume content that triggered the flag.</p>

  <p>All agent decisions are logged via the ReAct tracing system in <code data-type="file">src/ai/orchestrator/tracing.ts</code>. Each trace entry records the agent ID, name, start/end timestamps, status (running/success/failed/skipped), and a sequence of steps. Each step contains three fields: thought (the agent&rsquo;s reasoning), action (the function called), and observation (the result). This enables employers to inspect the complete decision chain for any candidate score.</p>

  <h4 class="subsection-title">5.3.5 Skills Taxonomy</h4>

  <p>The skills taxonomy in <code data-type="file">src/ai/taxonomy/skillsGraph.ts</code> (1540 lines) defines 186 skill nodes organized into 10 categories: language, framework, library, database, devops, cloud, tool, methodology, soft-skill, and domain. Each node specifies a canonical ID, display name, aliases (for normalization), category, and adjacent skills (for adjacency matching). The <code data-type="fn">computeSkillOverlap()</code> function computes a composite score: (exact_matches &times; 1.0 + adjacent_matches &times; 0.5) / total_jd_skills.</p>
</div>
...
<div class="report-page">
  ...
  <p>Employers create assessment criteria through the <code data-type="type">CriteriaPublishForm</code> component, which populates a <code data-type="type">BridgeCriteria</code> object. The criteria include: job title, description, required and preferred skills (populated from the skills taxonomy autocomplete), custom scoring signals with individual weights, a global threshold score, and test configuration specifying which skills to verify, the difficulty floor, and question count per skill. Published criteria are stored in Firestore with a unique short code and expiry date.</p>

  <h4 class="subsection-title">5.4.2 Self-Assessment</h4>

  <p>Before taking the Bridge test, candidates complete a self-assessment via <code data-type="file">src/bridge/hooks/useSelfAssessment.ts</code>. The self-assessment runs the employer scoring pipeline against the published criteria, producing a <code data-type="type">ScoreBreakdown</code> that shows how the candidate&rsquo;s resume scores on each criterion. This transparency allows candidates to see their strengths and gaps before entering the timed assessment.</p>

  <h4 class="subsection-title">5.4.3 Adaptive Testing Engine</h4>

  <p>The testing engine implements computerized adaptive testing (CAT) with 5 difficulty levels. The <code data-type="fn">getNextLevel()</code> function in <code data-type="file">src/bridge/test/adaptiveScoring.ts</code> adjusts difficulty: a correct answer advances the level by 1 (capped at 5); an incorrect answer after one wrong at the current level drops by 1 (floored at 1). Level multipliers reward higher-difficulty correct answers: L1=1.0, L2=1.5, L3=2.5, L4=4.0, L5=6.0. A sustained performance bonus of 15% applies when the candidate answers 3 or more consecutive questions correctly at level 3 or above.</p>
  ...
  <p>Questions are generated by <code data-type="file">src/bridge/test/questionGenerator.ts</code> using Gemini 2.0 Flash with structured JSON output. Three question types rotate: concept (3s answer buffer), scenario (5s buffer), and micro-challenge (8s buffer). Time allotment is computed as: (wordCount / candidateWpm) &times; 60 + buffer, modified by level-specific time modifiers.</p>

  <h4 class="subsection-title">5.4.4 Anti-Cheat System</h4>

  <p>The anti-cheat system operates on two layers. The browser integrity layer (<code data-type="file">src/bridge/test/antiCheat.ts</code>) monitors: tab switches via <code>visibilitychange</code> and <code>blur</code> events (5-point penalty, with 500ms deduplication window), paste events (8-point penalty), and fullscreen exits (3-point penalty). Speed anomaly detection flags impossibly fast correct answers: ratio &lt; 0.3 yields a 10-point penalty, with a 15-point compound anomaly penalty if the tab was hidden within the last 10 seconds.</p>

  <p>The audio intelligence layer (<code data-type="file">src/bridge/test/audioMonitor.ts</code>) implements three analysis tiers: (1) spectral analysis checking for formant energy concentration &gt;50% in the 300&ndash;3400 Hz speech band, (2) temporal analysis detecting syllabic modulation at 3&ndash;8 Hz via zero-crossing rate of the energy envelope, and (3) adaptive baseline recalibration using a 60-second rolling window of non-speech samples. Speech is declared when formants and syllabic modulation are both present and the RMS dB exceeds baseline by 3 dB. Impulses under 200ms are ignored (hammer/cough filter). Penalties: speech burst 2&ndash;5s (1 point), conversation (3+ bursts in 30s window, 5 points), continuous speech &gt;10s (8 points), whisper detection (6 points), speech coinciding with tab switch (12 points).</p>
  ...
  <p>After test completion, the scorecard is assembled with verification scores, integrity scores, resume pin, calibration data, and gap analysis (difference between self-assessed resume score and verified skill score). The scorecard is signed via HMAC-SHA256 using per-criteria secrets generated server-side in Firebase Cloud Functions. The <code data-type="type">SignedScorecard</code> interface (defined in <code data-type="file">src/bridge/types.ts</code>) includes a version number, criteria hash, and the HMAC signature, enabling third-party verification that the scorecard was produced by the platform and has not been modified.</p>

  <h4 class="subsection-title">5.4.6 Matching System</h4>

  <p>When a candidate&rsquo;s scorecard meets the employer&rsquo;s threshold, a <code data-type="type">MatchSignal</code> is created containing the criteria code, scorecard reference, candidate contact information, resume score, verified score, integrity score, and gap. Employers view matches through the <code data-type="type">EmployerMatchDashboard</code> component and can respond via the <code data-type="type">EmployerReply</code> system. Match status tracks progression through pending, viewed, and replied states.</p>

  <h3 class="section-title">5.5 Wellbeing Scoring Engine</h3>

  <p>The wellbeing engine is implemented in <code data-type="file">src/wellbeing/engine/wellbeingScorer.ts</code> with individual sub-score formulas in <code data-type="file">src/wellbeing/engine/formulas.ts</code>. The engine accepts a <code data-type="type">WellbeingInput</code> with commute minutes, work hours per week, work mode, offered salary, office city, candidate city, industry, commute mode, relocation flag, and optional driving distance.</p>
  ...
  <p>The device fingerprinting system in <code data-type="file">src/firebase/deviceId.ts</code> generates a deterministic hardware-based identifier by combining 11 signals: screen dimensions and color depth, device pixel ratio, CPU core count, device memory, platform string, timezone, language, max touch points, WebGL vendor and renderer (via <code>WEBGL_debug_renderer_info</code>), canvas fingerprint (GPU-specific rendering of text and arcs), and audio context signature (sample rate and max channel count). These signals are concatenated with a <code data-type="str">||</code> delimiter and hashed via SHA-256 to produce a 32-character device ID prefixed with <code data-type="str">dev_</code>.</p>

  <p>The fingerprint is stored in both localStorage (fast path) and IndexedDB (resilience). The <code data-type="fn">verifyDevice()</code> function regenerates the fingerprint from current hardware signals and compares against a stored ID, enabling cross-session identity verification without cookies.</p>
...
<div class="report-page">
  ...
  <p>Tests are co-located with their source modules in <code data-type="file">__tests__/</code> directories, following the convention <code data-type="file">ModuleName.test.ts</code> or <code data-type="file">ModuleName.test.tsx</code> for component tests. Each test file imports from the module under test without mocking internal dependencies, ensuring that tests validate actual behavior rather than assumed interfaces.</p>

  <h3 class="section-title">6.2 Unit Testing</h3>

  <h4 class="subsection-title">6.2.1 Saathi Engine Tests</h4>

  <p>The Saathi engine test suite spans multiple files covering: slot state management (<code data-type="file">slots.test.ts</code>) verifying slot creation, filling, phase transitions, and percentage calculations; resume generation (<code data-type="file">resumeGenerator.test.ts</code>) confirming correct conversion from slots to Resume objects with multiple entries; slot machine integration (<code data-type="file">slotMachine.test.ts</code>) testing full conversation flows from greeting through review; and language detection (<code data-type="file">languageDetect.test.ts</code>) verifying Unicode script identification for all 10 supported scripts. End-to-end LLM extraction is validated against live Gemma 3 27b and Gemini 2.5 Flash endpoints across an eight-scenario suite covering name, location, education, work history, skills, multilingual negation, confusion, and combined contact-detail replies, with both models scoring 100 percent on the suite.</p>

  <h4 class="subsection-title">6.2.2 AI Pipeline Tests</h4>

  <p>The AI pipeline test suite includes: L1 NLP agent tests (<code data-type="file">L1_NLPAgent.test.ts</code>) verifying section detection, entity extraction, TF-IDF vectorization, Jaccard similarity, and blended skills scoring; scoring tests (<code data-type="file">scoring.test.ts</code>) validating the composite scoring formula, weight normalization, distance decay, GPA scoring, VALUE rubric classification, and red flag penalty application.</p>

  <h4 class="subsection-title">6.2.3 Bridge Tests</h4>

  <p>Bridge testing covers: resume pinning (<code data-type="file">resumePin.test.ts</code>) verifying SHA-256 hashing, content normalization, n-gram Jaccard change detection, and change classification thresholds; store operations (<code data-type="file">store.test.ts</code>) testing test session lifecycle, flag recording, question advancement, and scorecard storage; adaptive scoring (<code data-type="file">adaptiveScoring.test.ts</code>) validating level transitions, score ceiling enforcement, sustained performance bonuses, and integrity score computation; and question generation (<code data-type="file">questionGenerator.test.ts</code>) testing anti-LLM-tell validation, time allotment calculation, and option shuffling.</p>

  <h4 class="subsection-title">6.2.4 Wellbeing Tests</h4>

  <p>Wellbeing testing spans 3 files: formula tests (<code data-type="file">formulas.test.ts</code>) verifying all 8 sub-score functions against known input-output pairs including boundary conditions; Maps client tests (<code data-type="file">mapsClient.test.ts</code>) validating distance API integration and fallback behavior; and wellbeing scorer integration tests (<code data-type="file">wellbeingScorer.test.ts</code>) confirming composite score computation, weight application, relocation penalties, and classification thresholds.</p>

  <h4 class="subsection-title">6.2.5 UI Component Tests</h4>

  <p>Component tests use Testing Library to validate user interactions: Landing page tests (<code data-type="file">Landing.test.tsx</code>) verify navigation and accessibility landmarks; Candidate Dashboard tests (<code data-type="file">CandidateDashboard.test.tsx</code>) validate score display and interaction flows; SaathiChat tests (<code data-type="file">SaathiChat.test.tsx</code>) confirm message rendering, input handling, and voice button states; Layout tests (<code data-type="file">Layout.test.tsx</code>) verify responsive navigation and theme toggling; builder template tests (<code data-type="file">templates.test.tsx</code>) ensure all four templates render without errors; AI Coach tests (<code data-type="file">AICoachPanel.test.tsx</code>) validate suggestion display and citation rendering; and theme tests (<code data-type="file">tokens.test.ts</code>) verify design token consistency.</p>
...
<div class="report-page">
  ...
  <p><strong>Challenge 6: Service Worker Caching of ML Models.</strong> ONNX model files (20&ndash;600MB) caused the service worker to exceed the browser&rsquo;s cache storage quota when aggressively precached. <em>Solution:</em> ML models were excluded from the service worker precache manifest. Instead, the model loaders in <code data-type="file">src/ai/models/onnx.ts</code> and <code data-type="file">src/ai/models/webllm.ts</code> store downloaded models in IndexedDB, which has significantly higher storage limits. The service worker only caches application code, stylesheets, and small static assets.</p>

  <p><strong>Challenge 7: Map Serialization in Zustand Stores.</strong> The Saathi <code data-type="type">SlotState</code> uses JavaScript <code data-type="type">Map</code> and <code data-type="type">Set</code> objects, which are not JSON-serializable. IndexedDB persistence and localStorage serialization silently dropped Map/Set data. <em>Solution:</em> Custom serialization/deserialization functions (<code data-type="fn">serializeState()</code> / <code data-type="fn">deserializeState()</code> in <code data-type="file">slotMachine.ts</code>) convert Maps to <code data-type="str">[key, value][]</code> arrays and Sets to string arrays before JSON.stringify, then reconstruct the original data structures on load.</p>
...
<div class="report-page">
  ...
  <ol>
    <li><strong>Conversational Resume Building:</strong> Saathi introduces a slot-filling dialogue architecture with 23 data slots across 7 phases, supporting voice input in 10 Indian languages. A pure language-model pipeline (Gemma 3 27b primary, Gemini 2.5 Flash backup) handles extraction and response generation, passing the full conversation history with each turn so the assistant never re-asks a question the user has already answered.</li>
...
<div class="report-page">
  ...
  <table class="project-diary">
    ...
    <tbody>
      ...
      <tr><td>14</td><td>Apr 7&ndash;13</td><td>Saathi AI mode: Gemma 3 27b primary with Gemini 2.5 Flash backup for extraction and response generation, single-source-of-truth model config, no-repeat conversation history.</td><td>AI-driven Saathi</td></tr>
...
<div class="report-page">
  ...
  <p>The skills taxonomy defined in <code data-type="file">src/ai/taxonomy/skillsGraph.ts</code> contains 186 skill nodes organized into 10 categories. Each node includes a canonical ID, display name, aliases for normalization, and adjacent skills for proximity matching. The taxonomy is inspired by ESCO (European Skills/Competences), O*NET (US Occupational Information Network), and LinkedIn Skills Graph, tuned for Indian fresher and tech hiring contexts.</p>
