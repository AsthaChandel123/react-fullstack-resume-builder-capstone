...
<div class="report-page">
  ...
  <p>Conversation flow proceeds through 7 phases defined by the <code data-type="type">ConversationPhase</code> type: warmup, education, experience, projects, skills, wrapup, and review. Phase transitions are governed by the <code data-type="fn">updatePhase()</code> function in <code data-type="file">src/saathi/engine/slotMachine.ts</code>, which examines slot completion state to determine the appropriate next phase. Users may skip non-required phases through negation detection (<code>NEGATION_RE</code> pattern: &ldquo;no&rdquo;, &ldquo;none&rdquo;, &ldquo;skip&rdquo;, &ldquo;nahi&rdquo;, &ldquo;nhi&rdquo;, &ldquo;not applicable&rdquo;).</p>

  <p>The engine operates in dual processing modes. In offline mode, <code data-type="fn">processUserInput()</code> performs synchronous entity extraction via regex patterns in <code data-type="file">src/saathi/engine/entityExtractor.ts</code> and fills slots through pattern matching for names, locations, institutions, skills, and experience details. In AI-enhanced mode, <code data-type="fn">processUserInputAsync()</code> invokes Gemini 2.5 Flash through <code data-type="file">src/saathi/engine/aiExtractor.ts</code> for natural language understanding of Hinglish and regional language inputs, with structured JSON output conforming to the <code data-type="type">AIExtractedData</code> schema.</p>

  <p>The response generation system maintains 220 templates organized across 24 response categories in <code data-type="file">src/saathi/engine/responseBank.ts</code>. Templates support variable interpolation (name, location, degree, institution, year, field, company, role, skills_list, project) and rotate through variants using a round-robin <code data-type="fn">pickVariant()</code> function to avoid repetitive responses.</p>
...
<div class="report-page">
  ...
  <p>Each user message is processed through the following pipeline: (1) input sanitization (2000-character limit, control character removal), (2) entity extraction via <code data-type="fn">extractEntities()</code>, (3) slot filling through <code data-type="fn">fillSlotsFromEntities()</code> which maps extracted entities to appropriate slot IDs, (4) phase update via <code data-type="fn">updatePhase()</code>, and (5) response generation via <code data-type="fn">generateResponse()</code> which selects the appropriate template based on what was just filled.</p>
  ...
  <p>Entity extraction in <code data-type="file">src/saathi/engine/entityExtractor.ts</code> employs a two-tier approach. The regex tier defines 12 pattern categories: email (<code>EMAIL_RE</code>), Indian phone numbers (<code>PHONE_RE</code>), international phone (<code>PHONE_INTL_RE</code>), years (<code>YEAR_RE</code>), date ranges (<code>DATE_RANGE_RE</code>), CGPA/GPA (<code>CGPA_RE</code>), percentages (<code>PERCENTAGE_RE</code>), GPA fractions (<code>GPA_FRACTION_RE</code>), degrees (<code>DEGREE_RE</code> matching 17 degree patterns), LinkedIn URLs, GitHub URLs, and a 68-entry skill vocabulary set (<code>SKILL_WORDS</code>).</p>

  <p>The enhanced extraction function <code data-type="fn">extractEntitiesEnhanced()</code> augments regex results with DistilBERT-NER predictions from <code data-type="file">src/saathi/engine/nerModel.ts</code>. The NER model (Xenova/distilbert-NER, INT8 quantized) identifies PER (person name), ORG (organization), LOC (location), and MISC entities. Predictions with confidence below 0.5 are discarded. NER results are merged into the <code>properNouns</code> array of the <code data-type="type">ExtractedEntities</code> interface for downstream processing by the slot filler.</p>
  ...
  <p>When the Gemini API key is available, <code data-type="fn">processUserInputAsync()</code> in the slot machine invokes two AI services sequentially: (1) <code data-type="fn">extractWithAI()</code> sends the user message, conversation context (last 4 exchanges), and current phase to Gemini 2.5 Flash with a structured JSON output schema (<code data-type="type">AI_EXTRACTION_SCHEMA</code>) for entity extraction; (2) <code data-type="fn">generateSaathiResponse()</code> produces a contextual, warm response based on the extracted data, current phase, filled slots, and missing slots. Both functions fall back to regex-based extraction and template-based responses on any failure.</p>
...
<div class="report-page">
  ...
  <p>The Saathi engine test suite spans 6 test files covering: slot state management (<code data-type="file">slots.test.ts</code>) verifying slot creation, filling, phase transitions, and percentage calculations; entity extraction (<code data-type="file">entityExtractor.test.ts</code>) testing all 12 regex patterns including edge cases for Indian phone formats, CGPA variants, and skill detection; response bank (<code data-type="file">responseBank.test.ts</code>) validating template interpolation and variant rotation; resume generation (<code data-type="file">resumeGenerator.test.ts</code>) confirming correct conversion from slots to Resume objects with multiple entries; slot machine integration (<code data-type="file">slotMachine.test.ts</code>) testing full conversation flows from greeting through review; and language detection (<code data-type="file">languageDetect.test.ts</code>) verifying Unicode script identification for all 10 supported scripts.</p>
